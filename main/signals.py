...
def payment_notification(sender, **kwargs):
    ...
    if ipn_obj.payment_status == ST_PP_COMPLETED:
        if ipn_obj.receiver_email != "receiver_email@example.com":
            # Not a valid payment
            return
        order_exist = UserOrder.objects.filter(user=int(ipn_obj.custom), pending=1).exist()

        if order_exist:
            order = UserOrder.objects.filter(user=int(ipn_obj.custom), pending=1).order_by('-created').first().update(
                paypal_payer_id=ipn_obj.payer_id, paypal_order_id=ipn_obj.txn_id, product=ipn_obj.item_name, payed=1,
                pending=0, cancelled=0)
        else:
            order = UserOrder(user=int(ipn_obj.custom), paypal_payer_id=ipn_obj.payer_id,
                              paypal_order_id=ipn_obj.txn_id,
                              product=ipn_obj.item_name, payed=1, pending=0, cancelled=0)
            order.save()

        if ipn_obj.custom == "Gold Subscription":
            UserContact.objects.get(user=int(ipn_obj.custom)).update(normal_user=0, premium_user=0,
                                                                     super_premium_user=1)
        else:
            UserContact.objects.get(user=int(ipn_obj.custom)).update(normal_user=0, premium_user=0,
                                                                     super_premium_user=1)
